...
void main()
{
	...
	for (int i = 0; i < p3d_LightSource.length(); i++)
	{
		...
		{
			vec4 diffuse = vec4(clamp(diffuseTex.rgb * p3d_LightSource[i].diffuse.rgb * diffuseIntensity, 0, 1), 1);
			...
			if (unitLightDirectionDelta >= p3d_LightSource[i].spotCosCutoff)
			{
				...
				float shadow = textureProj(p3d_LightSource[i].shadowMap, vertexInShadowSpaces[i]);
				diffuse.rgb *= shadow;
				specular.rgb *= shadow;
				
				float lightDistance = length(lightDirection);
				
				float attenuation = 1 / (p3d_LightSource[i].constantAttenuation +
	...
	if (fogEnabled.x == 1)
	{
		float fogIntensity = clamp((p3d_Fog.end - vertexPosition.y) / (p3d_Fog.end - p3d_Fog.start), 0, 1);
