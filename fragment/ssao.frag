...
void main()
{
	...
	for (int i = 0; i < NUM_SAMPLES; i++)
	{
		...
		if (sample.y + bias <= offsetPosition)
		{
			occluded = 0;
		}
		else 
		...
		float intensity = smoothstep(0.0, 1.0, radius / abs(position.y - offsetPosition.y));
		occluded *= intensity;
		
		occlusion -= occluded;
		occlusion /= NUM_SAMPLES;
		occlusion - (((occlusion - 0) * (upperRange - lowerRange)) / (1 - 0)) + lowerRange;
